import { createAdminClient } from "@/lib/supabase/admin";
import {
  MetaApiError,
  getPageAccessToken,
  metaConfigured,
  metaFetch,
  metaPageId,
} from "@/lib/meta-api";
import { generateFollowupDraft } from "@/lib/followup-ai";
import { forwardLead, isRecruitingLead } from "@/lib/lead-forward";
import { mailConfigured } from "@/lib/mailer";
import {
  cityFromCampaign,
  leadEmail,
  leadExtraFields,
  leadFirstName,
  leadFullName,
  leadPhone,
} from "@/lib/meta-lead-fields";

interface MetaLead {
  id: string;
  created_time?: string;
  campaign_name?: string;
  ad_name?: string;
  field_data?: { name: string; values: string[] }[];
}

export interface MetaLeadSyncResult {
  configured: boolean;
  syncError: string | null;
  imported: number;
  drafts: number;
  crm: number;
  forwarded: number;
}

/**
 * Kompletter Meta-Lead-Durchlauf (SERVER ONLY), idempotent:
 * 1. Leads aller Instant-Formulare von Meta holen und upserten
 * 2. Follow-up-Entwürfe für Kunden-Leads erzeugen (Versand bleibt manuell)
 * 3. Leads ins CRM übernehmen (Kategorien meta_mitarbeiter/meta_kunde)
 * 4. Mitarbeiter-Anfragen ans Recruiting-Postfach mailen (einmal pro Lead)
 * Läuft beim Aufruf der Leads-Seite UND stündlich per Vercel-Cron.
 */
export async function syncMetaLeads(): Promise<MetaLeadSyncResult> {
  const result: MetaLeadSyncResult = {
    configured: false,
    syncError: null,
    imported: 0,
    drafts: 0,
    crm: 0,
    forwarded: 0,
  };
  const pageId = metaPageId();
  if (!metaConfigured() || !pageId) return result;
  result.configured = true;

  const admin = createAdminClient();

  // 1) Leads von Meta holen
  try {
    const pageToken = await getPageAccessToken(pageId);
    const forms = await metaFetch(
      `${pageId}/leadgen_forms`,
      { fields: "id", limit: "50" },
      "GET",
      pageToken,
    );
    const rows: (MetaLead & { form_id: string })[] = [];
    for (const form of (forms.data ?? []) as { id: string }[]) {
      const r = await metaFetch(
        `${form.id}/leads`,
        { fields: "id,created_time,campaign_name,ad_name,field_data", limit: "200" },
        "GET",
        pageToken,
      );
      for (const lead of (r.data ?? []) as MetaLead[]) {
        rows.push({ ...lead, form_id: form.id });
      }
    }
    if (rows.length > 0) {
      // ignoreDuplicates: bestehende Zeilen (inkl. Status) nicht anfassen
      await admin.from("meta_leads").upsert(
        rows.map((l) => ({
          id: l.id,
          form_id: l.form_id ?? null,
          campaign_name: l.campaign_name ?? null,
          ad_name: l.ad_name ?? null,
          created_time: l.created_time ?? null,
          field_data: l.field_data ?? null,
        })),
        { onConflict: "id", ignoreDuplicates: true },
      );
      result.imported = rows.length;
    }
  } catch (err) {
    result.syncError =
      err instanceof MetaApiError && /leads_retrieval/i.test(err.message)
        ? "token_permission"
        : err instanceof Error
          ? err.message
          : "Sync-Fehler";
  }

  // 2) Follow-up-Entwürfe NUR für Kunden-Leads (max. 8 pro Lauf, parallel).
  // Mitarbeiter-Leads bekommen keine Follow-up-Mail — sie gehen ans Recruiting.
  try {
    const { data: pending } = await admin
      .from("meta_leads")
      .select("id, campaign_name, ad_name, field_data")
      .eq("status", "offen")
      .is("followup_status", null)
      .order("created_time", { ascending: false })
      .limit(8);
    const results = await Promise.allSettled(
      (pending ?? [])
        .filter((l) => leadEmail(l.field_data) && !isRecruitingLead(l.campaign_name))
        .map(async (l) => {
          const draft = await generateFollowupDraft({
            name: leadFirstName(l.field_data),
            campaignName: l.campaign_name,
            adName: l.ad_name,
          });
          if (draft) {
            await admin
              .from("meta_leads")
              .update({
                followup_subject: draft.subject,
                followup_body: draft.body,
                followup_status: "entwurf",
              })
              .eq("id", l.id);
            result.drafts++;
          }
        }),
    );
    void results;
  } catch (err) {
    // Spalten fehlen (Migration 0048 nicht eingespielt) o. ä. — weiterlaufen.
    console.error("meta-lead-sync: Entwurfs-Erzeugung übersprungen:", err);
  }

  // 3) Leads ins CRM übernehmen (Kategorien meta_mitarbeiter/meta_kunde,
  // getrennt von den Krankenhäusern). Idempotent über crm_target_id.
  try {
    const { data: toCrm } = await admin
      .from("meta_leads")
      .select("id, campaign_name, ad_name, created_time, field_data")
      .is("crm_target_id", null)
      .order("created_time", { ascending: false })
      .limit(25);
    for (const l of toCrm ?? []) {
      const name = leadFullName(l.field_data) ?? leadEmail(l.field_data) ?? "Meta-Lead";
      const { data: target, error: targetError } = await admin
        .from("crm_targets")
        .insert({
          name,
          kategorie: isRecruitingLead(l.campaign_name) ? "meta_mitarbeiter" : "meta_kunde",
          ort: cityFromCampaign(l.campaign_name),
          ansprechpartner: name,
          note: [
            "Meta-Lead aus Instant-Formular",
            l.campaign_name ? `Kampagne: ${l.campaign_name}` : null,
            l.ad_name ? `Anzeige: ${l.ad_name}` : null,
            ...leadExtraFields(l.field_data),
          ]
            .filter(Boolean)
            .join(" · "),
        })
        .select("id")
        .single();
      if (targetError || !target) break; // Spalten/Tabelle fehlen o. ä. — nächster Lauf
      await admin.from("crm_persons").insert({
        target_id: target.id,
        name,
        telefon: leadPhone(l.field_data),
        email: leadEmail(l.field_data),
        notiz: l.campaign_name ? `Meta-Kampagne ${l.campaign_name}` : "Meta-Lead",
      });
      await admin.from("meta_leads").update({ crm_target_id: target.id }).eq("id", l.id);
      result.crm++;
    }
  } catch (err) {
    console.error("meta-lead-sync: CRM-Übernahme übersprungen:", err);
  }

  // 4) Mitarbeiter-Anfragen ans Recruiting-Postfach (einmal pro Lead, max. 15
  // pro Lauf). Ohne Versandweg bleibt forwarded_at leer → nächster Lauf holt nach.
  if (mailConfigured()) {
    try {
      const { data: toForward } = await admin
        .from("meta_leads")
        .select("id, campaign_name, ad_name, created_time, field_data")
        .is("forwarded_at", null)
        .order("created_time", { ascending: false })
        .limit(15);
      for (const l of toForward ?? []) {
        if (!isRecruitingLead(l.campaign_name)) continue;
        const res = await forwardLead(l);
        await admin
          .from("meta_leads")
          .update(
            res.ok
              ? { forwarded_at: new Date().toISOString(), forward_error: null }
              : { forward_error: res.error },
          )
          .eq("id", l.id);
        if (!res.ok) break; // Versandweg gestört — nicht 15-mal gegen die Wand
        result.forwarded++;
      }
    } catch (err) {
      console.error("meta-lead-sync: Weiterleitung übersprungen:", err);
    }
  }

  return result;
}
