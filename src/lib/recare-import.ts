  ...
  try {
    ...
    const response = await client.messages.create({
      ...
      system:
        ...
        "'bestandskunde' nur bei reiner ORGANISATION einer bereits laufenden Versorgung ohne zusätzlichen Bedarf: Terminverschiebung, Rechnungsfrage, Beschwerde über Einsätze, Krankmeldung des Klienten. " +
        "WICHTIG — Abgrenzung zur Leistungserweiterung: Will ein bestehender Kunde MEHR Leistung (zusätzliche Leistungen wie Insulingabe/Wundversorgung/Behandlungspflege, höhere Frequenz, zusätzliche Betreuung, Versorgung für einen weiteren Angehörigen), dann ist das 'neuinteressent' — auch wenn die Person oder ein Angehöriger schon betreut wird. Solche Anrufe bringen zusätzlichen Umsatz und müssen bearbeitet werden. " +
